  ...
  factory: (require) => {
    ...
    function applyLocale() {
      ...
      if (root) {
        ...
        if (root.parentNode) {
          if (root.classList.contains('collapsed')) updateTabStatus(); else render()
          if (!animBusy) finalizePlacement()
        }
      }
    ...
    function barSize() {
      ...
      // Layout dimensions exclude transient scale animations and include borders.
      return { width: root.offsetWidth || 120, height: root.offsetHeight || 36 }
    }
    ...
    function measureBarWhileExpanded() {
      ...
      const size = { width: Math.max(1, root.offsetWidth), height: Math.max(1, root.offsetHeight) }
    ...
    async function refresh(force) {
      ...
      try {
        ...
        // A disposed instance must not update a later mount or detached DOM.
        if (root !== requestRoot) return
      ...
      if (root !== requestRoot || !root.parentNode) return
    ...
    async function apply(ctx) {
      ...
      const cleanup = () => {
        if (disposed) return
        disposed = true
        if (localeObserver) { localeObserver.disconnect(); localeObserver = null }
        if (root && root.getAnimations) root.getAnimations().forEach(animation => animation.cancel())
        refreshing = false
        animBusy = false
        window.removeEventListener(EVT_SET, onSet)
