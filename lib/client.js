...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function feedbackPrompt(index) {
      const n = FEEDBACK_PROMPTS.length
      ...
      try {
        ...
        if (raw) {
          ...
          return {
            ...
            promptIndex: typeof p.promptIndex === 'number' ? p.promptIndex : 0,
          }
        }
      } catch { /* ignore */ }
      return { lastAt: 0, count: 0, promptIndex: 0 }
    }
    ...
    function buildFeedbackBar() {
      ...
      const input = el('input', {
        ...
        placeholder: feedbackPrompt(state.promptIndex),
        'aria-label': '反馈',
        autocomplete: 'off',
      })
      ...
      const submit = () => {
        ...
        if (!url) return
        const next = { lastAt: Date.now(), count: st.count + 1, promptIndex: st.promptIndex + 1 }
        ...
        input.value = ''
        input.placeholder = feedbackPrompt(next.promptIndex)
      }
